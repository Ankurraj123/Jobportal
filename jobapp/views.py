...
def create_job_View(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            instance.user = user
            instance.is_published = True  # auto-publish — no admin approval needed
            instance.save()
            # for save tags
            form.save_m2m()
            messages.success(
                    request, 'Your job has been posted and is now live!')
...
def all_applicants_view(request, id):
    ...
    all_applicants = Applicant.objects.filter(job=id).select_related('user', 'job')
    job = get_object_or_404(Job, id=id, user=request.user.id)
...
@login_required(login_url=reverse_lazy('account:login'))
@user_is_employer
def update_applicant_status_view(request, applicant_id, new_status):
    """Allow employer to accept or reject an applicant."""
    ...
    label = {'accepted': 'Accepted ✅', 'rejected': 'Rejected ❌', 'pending': 'Reset to Pending'}.get(new_status)
    messages.success(request, f'{applicant.user.get_full_name()} — {label}')
    return redirect(request.META.get('HTTP_REFERER', reverse('jobapp:applicants', kwargs={'id': applicant.job.id})))
...
def applicant_details_view(request, id):
    ...
    skills_list = []
    if applicant.skills:
        skills_list = [s.strip() for s in applicant.skills.split(',') if s.strip()]

    context = {
        'applicant': applicant,
        'skills_list': skills_list,
        'cv_projects': applicant.cv_projects or [],
        'cv_certificates': applicant.cv_certificates or [],
        'cv_training': applicant.cv_training or [],
        'cv_achievements': applicant.cv_achievements or [],
        'cv_links': applicant.cv_links or {},
    }
